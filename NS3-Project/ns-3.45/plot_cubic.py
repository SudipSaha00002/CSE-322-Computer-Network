...
def generate_comparisons(prefix, xlabel, filename):
    ...
    def plot_metric(metric_key, ylabel, title, suffix):
        ...
        if ax.get_ylim()[1] == 0: ax.set_ylim(0, 1)
        plt.tight_layout()
    ...
    plot_metric("nrg", 'Energy Consumed (Joules)', f'Energy vs {xlabel}', 'energy')

if __name__ == "__main__":
    print("Generating Wireless Comparison Bar Plots...")
    ...
    generate_comparisons("speed", "Speed of Nodes (m/s)", "speed.txt")
    generate_comparisons("coverage", "Coverage Area Scale Multiplier", "coverage.txt")
